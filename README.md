...
The Real Estate Analyzer is a data-driven application designed to extract, analyze, and model real estate data. 
It consists of an ETL pipeline for web scraping, data preprocessing, and database management, along with a front-end and back-end system that allows users to filter, 
...
3. **SQL Database**:
   - The preprocessed data is then loaded into a **PostgreSQL** database.
...
7. **Price Prediction (Modeller Module)**:
...
8. **Flask Backend**:
   - The Flask backend serves the React frontend, handling requests and communicating with both the API and the machine learning modules (Analyzer and Modeller).
...
## Usage
To run the project you need to run individually the APP, API and the React Frontend. You can also run the ETL Pipeline.
...
# You can also run the ETL script to updated the DB
$ python etl.py
```
